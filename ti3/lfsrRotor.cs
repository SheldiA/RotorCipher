...
namespace ti3
{
    class lfsrRotor
    {
        ...
        private byte rcbRight, rcbMiddle, rcbLeft;
        private TI2.LFSR lfsr;
        private byte rs;
        private byte previousResult;

        public lfsrRotor(string password)
        {
            rcbRight = 2;
            rcbMiddle = 5;
            rcbLeft = 1;
        ...
        {
            byte result = message;
            result = UseRotor(rightRotor,rcbRight,result);
            result = UseRotor(middleRotor, rcbMiddle, result);
            result = UseRotor(leftRotor, rcbLeft, result);
        ...
        public byte DecryptByte(byte message)
        {
            ...
            result = UseRotorDecrypt(leftRotor, rcbLeft, result);
            byte prevRs = rs;
            result = UseRotorDecrypt(middleRotor, rcbMiddle, result);
            result = UseRotorDecrypt(rightRotor, rcbRight, result);
        ...
        private byte UseRotor(byte[] currRotor, byte currRotorRcb,byte message)
        {
            byte rcb = lfsr.GetByte(currRotorRcb);
            rs = (byte)((rs + previousResult + rcb) % 256);
            return currRotor[(message + rs) % 256];
          
        }

        private byte UseRotorDecrypt(byte[] currRotor, byte currRotorRcb, byte message)
        {
            byte result = 0;
            ...
            if (currRotorRcb == rcbLeft)
                rs = (byte)((rs + 3*previousResult + lfsr.GetByte(rcbLeft) + lfsr.GetByte(rcbMiddle) + lfsr.GetByte(rcbRight)) % 256);
            if (currRotorRcb == rcbMiddle)
                rs = (byte)((rs - previousResult - lfsr.GetByte(rcbLeft)) % 256);
            if (currRotorRcb == rcbRight)
                rs = (byte)((rs - previousResult - lfsr.GetByte(rcbMiddle)) % 256);
            
            return (byte)(result - rs);
        }
    }
}
